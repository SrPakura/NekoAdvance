...
class MemoryBlock extends MemoryView {
	constructor(size, cacheBits) {
		...
		this.PAGE_MASK = (1 << this.ICACHE_PAGE_BITS) - 1;
		this.icache = new Array(size >> this.ICACHE_PAGE_BITS);
	}
	invalidatePage(address) {
...
class ROMView extends MemoryView {
	constructor(rom, offset) {
		...
		this.PAGE_MASK = (1 << this.ICACHE_PAGE_BITS) - 1;
		this.icache = new Array(Math.ceil(rom.byteLength / (1 << this.ICACHE_PAGE_BITS)));
...
class BIOSView extends MemoryView {
	...
	initHLEVectors() {
		if (!this.real && this.buffer && this.buffer.byteLength >= 0x4000) {
			var u32 = new Uint32Array(this.buffer);
			// 0x00: Reset Vector -> Jumps to 0x08000000
			u32[0x00 >> 2] = 0xE59F0000; // ldr r0, [pc] (0x08)
			u32[0x04 >> 2] = 0xE12FFF10; // bx r0
			u32[0x08 >> 2] = 0x08000000; // literal pool

			// 0x18: Hardware IRQ Vector -> Standard GBA BIOS IRQ Dispatcher
			u32[0x18 >> 2] = 0xE92D500F; // stmfd sp!, {r0-r3, r12, lr}
			u32[0x1C >> 2] = 0xE3A00404; // mov r0, #0x04000000
			u32[0x20 >> 2] = 0xE2800C02; // add r0, r0, #0x200
			u32[0x24 >> 2] = 0xE5901008; // ldr r1, [r0, #8] (IE & IF)
			u32[0x28 >> 2] = 0xE0011821; // and r1, r1, r1, lsr #16
			u32[0x2C >> 2] = 0xE59F2010; // ldr r2, [pc, #0x10] (loads 0x03007FFC from 0x44)
			u32[0x30 >> 2] = 0xE5922000; // ldr r2, [r2] (user IRQ handler)
			u32[0x34 >> 2] = 0xE1A0E00F; // mov lr, pc
			u32[0x38 >> 2] = 0xE12FFF12; // bx r2 (call user handler)
			u32[0x3C >> 2] = 0xE8BD500F; // ldmfd sp!, {r0-r3, r12, lr}
			u32[0x40 >> 2] = 0xE25EF004; // subs pc, lr, #4
			u32[0x44 >> 2] = 0x03007FFC; // literal pool for user handler
		}
	}
	/**
	 ...
	 */
	_checkProtection() {
		return this.real && this.mmu && this.mmu.cpu && this.mmu.cpu.gprs[this.mmu.cpu.PC] >= 0x00004000;
	}
...
class BadMemory {
	constructor(mmu, cpu) {
		...
		this.PAGE_MASK = (1 << this.ICACHE_PAGE_BITS) - 1;
		this.icache = [{ thumb: new Array(1), arm: new Array(1), invalid: true }];
	}
	load8(offset) {
		return 0;
	}
...
class GameBoyAdvanceMMU {
	...
	clear() {
		...
		if (!this.bios) {
			...
			this.bios.real = false;
			this.bios.mmu = this;
		} else if (!this.bios.real) {
			this.bios.initHLEVectors();
		}
		this.memory = [
			this.bios,
			this.badMemory,
	...
	addressToPage(region, address) {
		...
		return address >> (memory ? memory.ICACHE_PAGE_BITS : 8);
	}
	accessPage(region, pageId) {
		var memory = this.memory[region] || this.badMemory;
		if (!memory || !memory.icache || memory.icache.length === 0) {
			return { thumb: new Array(1), arm: new Array(1), invalid: true };
		}
		var index = pageId % memory.icache.length;
		if (index < 0) index += memory.icache.length;
		var page = memory.icache[index];
		if (!page || page.invalid) {
			var pageBits = memory.ICACHE_PAGE_BITS || 8;
			page = {
				thumb: new Array(1 << pageBits),
				arm: new Array(1 << Math.max(1, pageBits - 1)),
				invalid: false
			};
			memory.icache[index] = page;
		}
		return page;
	}
