class GameBoyAdvanceInterruptHandler {
	...
	swi32(opcode) {
		var swiNum = opcode >> 16;
		if (swiNum === 0 && opcode !== 0) {
			swiNum = opcode & 0xff;
		}
		this.swi(swiNum);
	}
	swi(opcode) {
		...
		switch (opcode) {
			...
			case 0x02:
				...
				if (!this.enable) {
					...
					while (!this.interruptFlags) {
						this.pollNextEvent();
						if (!this.nextEvent || this.nextEvent <= this.cpu.cycles) {
							this.cpu.cycles += 100;
							this.updateTimers();
							break;
						}
			...
			case 0x05:
				// VBlankIntrWait
				this.cpu.gprs[0] = 1;
				this.cpu.gprs[1] = this.MASK_VBLANK;
			// Fall through:
			case 0x04:
				// IntrWait
				if (!this.enable) {
				...
				if (this.cpu.gprs[0]) {
					this.dismissIRQs(this.cpu.gprs[1]);
				}
				if (!(this.interruptFlags & this.cpu.gprs[1])) {
					while (!(this.interruptFlags & this.cpu.gprs[1])) {
						this.pollNextEvent();
						if (!this.nextEvent || this.nextEvent <= this.cpu.cycles) {
							this.cpu.cycles += 100;
							this.updateTimers();
			...
			case 0x09:
				...
				this.cpu.gprs[0] = ((Math.atan(x) / (2 * Math.PI)) * 0x10000) | 0;
				break;
			case 0x0d:
				// BiosChecksum
			...
			case 0x19:
				// SoundBiasChange
				if (this.cpu.gprs[0]) {
					this.core.io.registers[this.core.io.SOUNDBIAS >> 1] = 0x200;
				} else {
					this.core.io.registers[this.core.io.SOUNDBIAS >> 1] = 0;
				}
				break;
			case 0x0a:
				// ArcTan2
