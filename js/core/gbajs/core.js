class ARMCore {
	...
	fetchPage(address) {
		...
		if (region == this.pageRegion) {
			...
			var mem = this.mmu.memory[region] || this.mmu.badMemory;
			this.pageMask = mem ? mem.PAGE_MASK : 0x3ff;
	...
	loadInstructionArm(address) {
		...
		var offset = (address & (this.pageMask || 0x3ff)) >> 2;
		if (offset >= this.page.arm.length) offset = offset % this.page.arm.length;
	...
	loadInstructionThumb(address) {
		var next = null;
		this.fetchPage(address);
		var offset = (address & (this.pageMask || 0x3ff)) >> 1;
		if (offset >= this.page.thumb.length) offset = offset % this.page.thumb.length;
	...
	badOp(instruction) {
		var cpu = this;
		var func = function () {
			cpu.WARN("Illegal instruction: 0x" + instruction.toString(16) + " at 0x" + (cpu.gprs[cpu.PC] - cpu.instructionWidth).toString(16));
		};
		func.writesPC = false;
	...
	compileArm(instruction) {
		...
		if ((instruction & 0x0ffffff0) == 0x012fff10) {
			...
			if ((opcode & 0x01800000) == 0x01000000 && !s) {
				...
				if ((instruction & 0x00b00000) == 0x00200000 || (instruction & 0x02000000)) {
